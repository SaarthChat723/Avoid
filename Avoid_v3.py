...
def gameLoop():
 ...
 #           gameOver = True
        ...
        y += y_change
        screen.fill(black)
        main = pygame.Rect(x,y,10,10)
        obs_1 = pygame.Rect(obj_x, obj_y, 20, 20)
        obs_2 = pygame.Rect(obj2_x, obj2_y, 20, 20)
        
        ob_1 = pygame.draw.rect(screen,white, [obj_x,obj_y,20,20])
        ob_2 = pygame.draw.rect(screen,white, [obj2_x,obj2_y,20,20])
        player = pygame.draw.rect(screen, red, [x,y,block_size,block_size])
#        showLives(lives)
        pygame.display.update()
        if main.colliderect(obs_1) or main.colliderect(obs_2):
            lives-=1
        if lives == 0:
            gameOver = True
    clock.tick(FPS)
